...
function displayClassName(value: string) {
  ...
  if (normalized === 'preprimary 1') return 'PP1'
  if (normalized === 'preprimary 2') return 'PP2'
...
function classOrderIndex(value: string) {
  const displayName = displayClassName(value)
  const index = CLASS_ORDER.indexOf(displayName)
  ...
  const stats = useMemo(() => {
    return data.classes
      .map((c) => {
        ...
        return {
          ...c,
          displayName: displayClassName(c.name),
          count,
          teacher,
        }
      })
      .sort(
        (a, b) =>
          classOrderIndex(a.displayName) -
          classOrderIndex(b.displayName)
      )
  }, [data])
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {group.classes.map((groupClassName) => {
              const classData = stats.find(
                (c) =>
                  normalizeClassName(c.displayName) ===
