...
for (const className of SCHOOL_CLASSES) {
  result[className] = data.students.filter((student) => {
    ...
    return studentClass === className.toLowerCase()
  })
}
...
link.href = url
link.download =
  className.replace(/\s+/g, '_') + '_Class_List.csv'
...
const filteredClasses = SCHOOL_CLASSES.filter((className) => {
...
<PageHeader title="Class Lists" description="Separate student lists for every class from Play Group to Grade 9." />
  ...
  <div className="space-y-3">
    {filteredClasses.map((className) => {
      const students = studentsByClass[className]
      const isOpen = openClasses.includes(className)
        ...
        >
          <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button
              ...
              onClick={() =>
                downloadClassList(className, students)
              }
            >
            ...
            <div className="border-t">
              {students.length === 0 ? (
                <div className="p-8 text-center text-sm text-muted-foreground">
                  No students are registered in {className}.
                </div>
              ) : (
